...
pub async fn run_stdio(server: HostRuntime) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 只构建一次 MCP dispatcher，使 stdio 仅依赖协议适配入口。
    ...
    loop {
        ...
        let response = handle_stdio_message(&dispatcher, &message, &mut session_state).await?;
...
async fn handle_stdio_message(
    ...
    if is_initialize_request(message) {
        let Some(response) = dispatcher.handle_message(message).await else {
