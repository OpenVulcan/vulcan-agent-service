...
use crate::config::HOST_CONFIG_FORMAT_VERSION;
...
impl Config {
    ...
    pub fn from_file(path: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if config.format_version != HOST_CONFIG_FORMAT_VERSION {
            return Err(format!(
                "Unsupported config format_version {}; expected {}",
                config.format_version, HOST_CONFIG_FORMAT_VERSION
            )
            .into());
        }
