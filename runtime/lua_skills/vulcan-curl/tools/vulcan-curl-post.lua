--[[
vulcan-curl-post
Execute one simple POST request through lua-curl using AI-friendly structured input.
通过 lua-curl 使用面向 AI 的结构化输入执行一次简单 POST 请求。
]]

-- Load the shared HTTP helper module from the current tool directory.
-- 从当前工具目录加载共享 HTTP 辅助模块。
local function load_shared_http()
    local entry_dir = tostring(vulcan.entry_dir or ".")
    local helper_path = vulcan.path_join(entry_dir, "shared_http.lua")
...
-- Run the simple POST tool with a protected boundary so output stays stable.
-- 使用保护边界执行简单 POST 工具，保证输出保持稳定。
return function(args)
    local helpers = load_shared_http()

    local ok, result = xpcall(function()
        local base_dir = tostring((args and args.cwd) or helpers.resolve_runtime_cwd())
        local spec = helpers.build_post_spec(args or {}, base_dir)
