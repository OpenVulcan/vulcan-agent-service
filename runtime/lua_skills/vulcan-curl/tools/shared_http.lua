...
local function resolve_local_path(base_dir, path_text)
    ...
    local join = vulcan and vulcan.path_join
...
local function resolve_runtime_cwd()
    if vulcan and type(vulcan.cwd) == "function" then
        local ok, cwd = pcall(vulcan.cwd)
...
local function read_text_file(path_text)
    if vulcan and type(vulcan.fs_read) == "function" then
        return vulcan.fs_read(path_text)
    end
    ...
    while index <= #argv do
        ...
        elseif value_options[token] then
            ...
            elseif field_name == "cookie" then
                ...
                local exists = vulcan and type(vulcan.fs_exists) == "function" and vulcan.fs_exists(resolved_candidate)
...
local function build_post_like_spec(args, base_dir, method_name)
    ...
    if args.json ~= nil then
        local json_encode = vulcan and vulcan.json_encode
