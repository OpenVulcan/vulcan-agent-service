...
local function resolve_client_budget_scope(vulcan_context)
    ...
    if type(budget) ~= "table" then
        return nil, {
            ...
            message = "vulcan.client_budget is required and must be injected by the MCP host",
        }
    end
    ...
    if type(scoped_budget) ~= "table" then
        return nil, {
            ...
            message = string.format("vulcan.client_budget.%s is required", scope_name),
            scope = scope_name,
        }
    end
    ...
    if not bytes_limit or bytes_limit <= 0 then
        return nil, {
            ...
            message = string.format("vulcan.client_budget.%s.bytes must be a positive number", scope_name),
            scope = scope_name,
    ...
    if lines_limit == nil then
        return nil, {
            ...
            message = string.format("vulcan.client_budget.%s.lines must be provided", scope_name),
            scope = scope_name,
