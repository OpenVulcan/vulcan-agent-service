...
local function load_directory_ignore_rules(directory_path)
    ...
    for _, ignore_file_name in ipairs({ ".gitignore", ".ignore" }) do
        local ignore_file_path = vulcan.path_join(directory_path, ignore_file_name)
        if vulcan.fs_exists(ignore_file_path) and not vulcan.fs_is_dir(ignore_file_path) then
            local ok, content = pcall(vulcan.fs_read, ignore_file_path)
            if ok then
...
local function get_current_working_directory()
    ...
    local command = vulcan.osinfo().os == "windows" and "cd" or "pwd"
...
local function resolve_scan_path(path)
    ...
    return vulcan.path_join(current_directory, normalized)
end
...
local function get_rule_path(language_key)
    ...
    local rule_path = vulcan.path_join(vulcan.path_join(get_skill_dir(), "rules"), language_spec.rule_file)
    if vulcan.fs_exists(rule_path) then
        return rule_path
    end
    return nil
end
    ...
    for line in tostring(output or ""):gmatch("[^\r\n]+") do
        ...
        if current ~= "" then
            if starts_with(current, "{") then
                local decoded, err = vulcan.json_decode(current)
                if decoded then
...
local function run_scan_batch(binary_directory, executable_name, rule_path, file_paths)
    if type(vulcan.exec) == "function" then
        local ok, result = pcall(vulcan.exec, {
            program = vulcan.path_join(binary_directory, executable_name),
...
local function collect_files_for_path(target_path, recursive, extension_filter, ignore_enabled)
    ...
    local function walk(current_directory, display_directory, inherited_ignore_rules)
        ...
        local entries = vulcan.fs_list(current_directory)
        if not entries then
            return
        end
        table.sort(entries)
        ...
        for _, entry in ipairs(entries) do
            if entry ~= "." and entry ~= ".." then
                local full_path = vulcan.path_join(current_directory, entry)
                local display_path = (display_directory == "." or display_directory == "") and entry or vulcan.path_join(display_directory, entry)
                local entry_is_directory = vulcan.fs_is_dir(full_path)
...
local function classify_target_path_modes(target_paths)
    ...
    for _, target_path in ipairs(target_paths or {}) do
        ...
        if not vulcan.fs_exists(resolved) then
            return nil, {
                error = "path_not_found",
                message = "path does not exist",
                path = target_path,
        ...
        if vulcan.fs_is_dir(resolved) then
            has_directory = true
        else
            has_file = true
        end
